...
public:
    ...
    public:
        ...
        enum Condition
        {
            ...
            CancelRSA,
            FailureOutOfDate,
            FailurePolicyViolation,      // 25
			FialureServerDisconnect,
			ServerSendPolicyViolation,    //服务器主动发送信令踢掉线直接到登录界面
			ServerSendPolicyViolationRelogin, //服务器主动发送指令踢掉然后重连
			ServerSendPolicyViolationReloginFromNav //服务器主动发送指令踢掉然后拿新的导航重连
        };
	...
	void setRealfrom(const QString&);

	QString realto() const;
	void setrealto(const QString&);
