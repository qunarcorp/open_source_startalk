...
#ifdef _DEBUG
#pragma comment(lib,"../3rd/cef/libcef_dll_wrapperd.lib")
#pragma comment(lib,"../3rd/cef/d/libcef.lib")
#else
#pragma comment(lib,"../3rd/cef/libcef_dll_wrapper.lib")
#pragma comment(lib,"../3rd/cef/r/libcef.lib")
#endif // DEBUG
...
bool QCefWebView::CreateBrowser(const QSize& size) {
  ...
  QString url = url_.isEmpty()||true ? kUrlBlank : url_.toString();
...
//     GetBrowser()->GetMainFrame()->LoadURL(cefurl);
      CefRefPtr<CefRequest> pReq = CefRequest::Create();
      CefString mothed = pReq->GetMethod();
      CefString cefurl(url_.toString().toStdWString());
      //pReq->SetMethod(CefString(QString("POST").toStdWString()));
      pReq->SetURL(cefurl);
      GetBrowser()->GetMainFrame()->LoadRequest(pReq);
    return true;
  }
  return false;
}

void QCefWebView::setCookie(const QMap<QString,QString> cookies,const QString& domain){
    CefRefPtr<CefCookieManager> manager = CefCookieManager::GetGlobalManager();
    for (QString key:cookies.keys())
    {
        if (!key.isEmpty() && !cookies.value(key).isEmpty())
        {
            CefCookie cookie;
            CefString(&cookie.name).FromWString(key.toStdWString());
            CefString(&cookie.value).FromWString(cookies.value(key).toStdWString());
            ...
            QString url = domain.indexOf("http://")==0||domain.indexOf("https://")==0 ? domain:"http://"+domain;

            CefPostTask(TID_IO, NewCefRunnableMethod(manager.get(), &CefCookieManager::SetCookie, CefString(url.toStdWString()), cookie));
        }
    }
}